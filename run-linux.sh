#!/usr/bin/env bash
# Cellmap Viewer launcher (Linux) — runs the Electron app.
# (No install needed just to VIEW data: open CellmapViewer.html in a browser.)
# First time: chmod +x run-linux.sh
set -e
cd "$(dirname "$0")"
...
BUNDLED="$PWD/tools/node/linux-$A/bin"
...
if ! command -v node >/dev/null 2>&1; then
  ...
  echo "    - drop a portable Node into tools/node/linux-$A/ ."
  echo
  exit 1
fi
